...
from data.thumb_data import build_thumb_data
from train.train_model1 import ThumbClassifier1 as thumb_classifier1

dependencies = ['torch']
model_path = {
    'thumb_classifier1': 'PD-Quant\custom2\finger_hand_classifier1.pth.tar'
}

def thumb_classifier1(pretrained=False, **kwargs):
    # Call the model, load pretrained weights
    model = thumb_classifier1(**kwargs)
    if pretrained:
        checkpoint = torch.load(model_path['thumb_classifier1'], map_location='cpu')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='resnet18', type=str, help='model name',
                        choices=['thumb_classifier1','resnet18', 'resnet50', 'mobilenetv2', 'regnetx_600m', 'regnetx_3200m', 'mnasnet'])
    parser.add_argument('--batch_size', default=64, type=int, help='mini-batch size for data loader')
    parser.add_argument('--workers', default=2, type=int, help='number of workers for data loader')
    parser.add_argument('--data_path', default='custom/data_custom', type=str, help='path to ImageNet data')
    ...
    parser.add_argument('--num_samples', default=64, type=int, help='size of the calibration dataset')
    parser.add_argument('--iters_w', default=100, type=int, help='number of iteration for adaround')
    ...
    if args.arch == 'thumb_classifier1':
        train_loader, test_loader = build_thumb_data(batch_size=args.batch_size, workers=args.workers,
